...
%token <number> FSM ENDF STATES WATCH ENDW STATE ENDS INITIALIZE ENDI IF ELSEIF ELSE ENDIF IFONCE ENDIFONCE NUMBER AND OR NOT TEAM SHIPS TEAMSHIPS SHIPSPOINT TEAMSPOINT VOLUMEPOINT PATH POINT VOLUME THISTEAM THISTEAMSHIPS THISTEAMSPOINT JUMP KW_TRUE KW_FALSE LT LTE EQ NEQ GTE GT FSMCREATE LOCALIZATION ENDL LSTRING
...
expression: expression '+' { fprintf(yyout, " + "); } expression
    ...
    |       KW_TRUE { fprintf(yyout, "1"); }
    |       KW_FALSE { fprintf(yyout, "0"); }
    ;
