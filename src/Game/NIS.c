...
bool_t nisScreenStarted = FALSE;

bool_t nisIsRunning = FALSE;
bool_t nisCaptureCamera = FALSE;
bool_t nisEnabled = TRUE;
Camera *nisCamera = NULL;

#if NIS_TIME_CONTROLS
real32 nisPlayFactor = 1.0f;
#endif
bool_t nisPaused = FALSE;

#if NIS_PRINT_INFO
char nisInfoString[256] = "";
bool_t nisPrintInfo = FALSE;
bool_t nisNoLockout = FALSE;
#endif
...
bool_t nisTaskToPause = FALSE;                    //task to be paused at end of loading
bool_t nisTaskToResume = FALSE;                   //task to be resumed at end of loading
...
bool_t nisFullyScissored = FALSE;                 //is scissor fully opaque?
bool_t bCameraFocussed;                           //set to TRUE if the camera was focussed on command of the script file
bool_t bPerfectFocusComputed;                     //set to TRUE when nisBlendCameraEndPoint called
bool_t nisSeeking = FALSE;                        //set to TRUE during seeks
bool_t nisNewNISStarted = FALSE;                  //set to TRUE when a new NIS is started by an event
...
----------------------------------------------------------------------------*/
bool_t nisShipStartPosition(vector *destVector, matrix *destMatrix, nisheader *header, ShipRace race, ShipType type, sdword instance)
{
    spaceobjpath *path;
