...
bool_t uicClearCurrent(regionhandle reg);
void uicSetCurrent(regionhandle reg, bool_t bUserInput);
...
/*-----------------------------------------------------------------------------
    ...
    Note        : can call user function with any region message if filter set
                    up properly.
----------------------------------------------------------------------------*/
bool_t uicButtonReleased = FALSE;
udword uicButtonProcess(regionhandle region, smemsize ID, udword event, udword data)
{
    udword mask = 0;
...
----------------------------------------------------------------------------*/
bool_t uicRepositionText(textentryhandle entry, bool_t jumpReposition)
{
    sdword length;
    fonthandle fhSave;
...
----------------------------------------------------------------------------*/
bool_t uicInsertCharacter(textentryhandle entry, udword character)
{
    sdword index;
    regVerify(&entry->reg);
...
----------------------------------------------------------------------------*/
bool_t uicBackspaceCharacter(textentryhandle entry)
{
    sdword index;
    regVerify(&entry->reg);
...
/*-----------------------------------------------------------------------------
    ...
    Return      : TRUE if the character was deleted properly
----------------------------------------------------------------------------*/
bool_t uicDeleteCharacter(textentryhandle entry)
{
    sdword index;
    regVerify(&entry->reg);
...
----------------------------------------------------------------------------*/
bool_t uicCursorWordLeft(textentryhandle entry)
{
    if (entry->iCharacter == 0)
    {                                                       //if can't move left
        return(FALSE);
...
/*-----------------------------------------------------------------------------
    ...
    Return      : TRUE if the cursor was moved.
----------------------------------------------------------------------------*/
bool_t uicCursorWordRight(textentryhandle entry)
{
...
udword uicTextEntryProcess(regionhandle reg, smemsize ID, udword event, udword data)
{
    ...
    regionhandle tabRegion;
    udword mask = 0;
    bool_t bShift = bitTest(data, RF_ShiftBit);
    int keyname;
