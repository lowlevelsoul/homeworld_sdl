...
bool_t processMoveToDo(CommandToDo *movetodo,bool_t passiveAttacked);
bool_t processMpHyperspaceingToDo(CommandToDo *movetodo);
bool_t processMoveLeaderToDo(CommandToDo *movetodo,bool_t passiveAttacked);
...
bool_t isSelectionExclusivlyStrikeCraft(SelectCommand *selection);
bool_t isSelectionExclusivlyStrikeCraftorNoMoveAndAttackShips(SelectCommand *selection);
bool_t selectionHasSwarmers(SelectCommand *selection);
...
extern bool_t BombersUseBombingRun;
...
/*-----------------------------------------------------------------------------
    ...
    Return      :
----------------------------------------------------------------------------*/
void PrepareShipsForCommand(CommandToDo *command,bool_t rowClear)
{
    sdword i;
    SelectCommand *selection = command->selection;
    Ship *ship;
...
/*-----------------------------------------------------------------------------
    ...
    Return      :
----------------------------------------------------------------------------*/
void PrepareOneShipForCommand(Ship *ship,CommandToDo *command,bool_t rowClear)
{
    ship->command = command;
    if ((rowClear) && (ship->specialFlags & SPECIAL_rowGettingOutOfWay))
    {
...
/*-----------------------------------------------------------------------------
    ...
    Return      :
----------------------------------------------------------------------------*/
void InitShipsForAI(SelectCommand *selection,bool_t fresh)
{
    sdword i;
    Ship *ship;
...
void InitShipsForAIDontResetAttack(SelectCommand *selection,bool_t fresh)
{
    sdword i;
    Ship *ship;
...
/*-----------------------------------------------------------------------------
    ...
    Return      :
----------------------------------------------------------------------------*/
void InitShipAI(Ship *ship,bool_t fresh)
{
    ship->aistate = 0;
    ship->aistateattack = 0;
    if(fresh)
...
void protectShip(Ship *ship,Ship *protectThisShip,bool_t passiveAttacked)
{
    vector diff;
    real32 dist;
    real32 desiredrange;
...
//    ship->shipidle = FALSE;
    ...
    dontrotate = ((passiveAttacked) & (bool_t)((ShipStaticInfo *)ship->staticinfo)->rotateToRetaliate);
...
/*-----------------------------------------------------------------------------
    ...
    Return      :
----------------------------------------------------------------------------*/
void protectShipsAvg(Ship *ship,ProtectCommand *protectShips,bool_t passiveAttacked)
{
    vector diff;
    real32 dist;
    real32 desiredrange;
...
----------------------------------------------------------------------------*/
bool_t processLaunchShipToDo(CommandToDo *launchtodo)
{
...
bool_t isThisShipThreateningMe(SpaceObjRotImpTarg *target,Ship *me)
{
...
----------------------------------------------------------------------------*/
bool_t EnemyIsBehindMe(SpaceObjRotImpTarg *enemy,Ship *me)
{
    vector myheading;
    vector enemytome;
...
----------------------------------------------------------------------------*/
bool_t processSpecialToDo(CommandToDo *todo)
{
...
/*-----------------------------------------------------------------------------
    ...
    Return      :
----------------------------------------------------------------------------*/
bool_t processHaltToDo(CommandToDo *command,bool_t passiveAttacked)
{
    SelectCommand *selection = command->selection;
    sdword numShips = selection->numShips;
    bool_t done = TRUE;
    ...
    {
        ship = selection->ShipPtr[i];
        if ((passiveAttacked) & (bool_t)((ShipStaticInfo *)ship->staticinfo)->rotateToRetaliate)
        {
...
----------------------------------------------------------------------------*/
bool_t ShipsInPassiveAttackRange(SelectCommand *selection,AttackCommand *attack)
{
    sdword numShips = selection->numShips;
    sdword numShipsToAttack = attack->numTargets;
    sdword i;
...
----------------------------------------------------------------------------*/
bool_t processPassiveAttackToDo(CommandToDo *attacktodo)
{
...
----------------------------------------------------------------------------*/
bool_t processAttackToDo(CommandToDo *attacktodo)
{
    sdword i;
    SelectCommand *selection = attacktodo->selection;
    AttackCommand *attack = attacktodo->attack;
...
bool_t delegateCommand(CommandToDo *attacktodo,sdword group,sdword doform, SelectCommand *globalselection)
{
    sdword i;
    SelectCommand *selection = attacktodo->selection;
    AttackCommand *attack = attacktodo->attack;
        ...
        else
        {
            if(ship->formationcommand->formation.flipselectionfixed == TRUE)
            {
                ...
                if(ship->formationcommand->formation.needFix == TRUE)
                {
                    ...
                    //negate this bool_t value so the formation status is uptodate.
...
bool_t targetIsntNearShip(Ship *ship,SpaceObjRotImpTarg *target)
{
    vector distvect;
    real32 dist;
...
/*-----------------------------------------------------------------------------
    ...
    Return      : returns TRUE if attack has been completed
----------------------------------------------------------------------------*/
bool_t processAttackToDoInFormation(CommandToDo *attacktodo)
{
    sdword donness = 0;
...
----------------------------------------------------------------------------*/
bool_t hyperspaceOutPingTimeOut(struct ping *hellaPing, SpaceObj *user, ubyte *userData, bool_t bRemoveReferences)
{
...
/*-----------------------------------------------------------------------------
    ...
    Return      : TRUE if the ping times out.
----------------------------------------------------------------------------*/
 bool_t hyperspaceInPingTimeOut(struct ping *hellaPing, SpaceObj *user, ubyte *userData, bool_t bRemoveReferences)
{
    if (bRemoveReferences)
    {
        if (user == pingDyingObject)
...
bool_t needToWaitForThisShip(Ship *ship)
{
    Node *node;
    CommandToDo *command;
...
bool_t processMoveToDo(CommandToDo *movetodo,bool_t passiveAttacked)
{
    ...
    for (i=0;i<numShips;i++)
    {
        ...
        if(ship->shiptype == ResearchShip)
        {
            ...
            {
                dontrotate = (passiveAttacked & (bool_t)shipstatic->rotateToRetaliate);
            }
        }
        else
        {
...
bool_t selectionHasStrikeCraft(SelectCommand *selection)
{
    sdword i;
...
void CalculateMoveToPoints(SelectCommand *selection,vector from,vector to)
{
    ...
    bool_t moveCalcsAllocated = FALSE;
...
bool_t selectionHasOtherForceNoAttackAndMoveShips(SelectCommand *selection)
{
    sdword i;
    for(i=0;i<selection->numShips;i++)
    {
...
/*-----------------------------------------------------------------------------
    ...
    Return      :
----------------------------------------------------------------------------*/
void InitExtraAttackInfo(SelectCommand *selection,AttackCommand *attack,bool_t findwingmen)
{
    sdword numShips = selection->numShips;
    sdword numTargetsToAttack = attack->numTargets;
    sdword i,j,k;
...
----------------------------------------------------------------------------*/
bool_t canChangeOrderToPassiveAttack(CommandToDo *alreadycommand,AttackCommand *attack)
{
...
bool_t ShipsAreSalCapCorvettes(Ship *ship)
{
...
bool_t ShipHasToLaunch(Ship *InsideShip, Ship *ship)
{
    if (singlePlayerGame)
    {
...
----------------------------------------------------------------------------*/
bool_t processBuildingShipToDo(CommandToDo *command)
{
    Player *player;
    Ship *creator;
...
----------------------------------------------------------------------------*/
bool_t FindShipsWereGuarding(CommandToDo *originalus,SelectCommand *us,GrowSelection *shipsWereGuarding,sdword iteration)
{
    sdword i,j;
...
/*-----------------------------------------------------------------------------
    ...
    Return      :
----------------------------------------------------------------------------*/
bool_t thereareothercompatibleresearchships(Ship *ship)
{
    Node *resnode;
    Ship *resship;
...
----------------------------------------------------------------------------*/
bool_t TargetWithinAttackRange(Ship *ship,SpaceObjRotImpTarg *target)
{
    vector trajectory;
    real32 range;
...
{
    bool_t needToPassiveAttack = FALSE;
    bool_t needToProtect = FALSE;
...
/*-----------------------------------------------------------------------------
    ...
    Return      :
----------------------------------------------------------------------------*/
bool_t salCapCorvettesInSelectionCantDoFormation(SelectCommand *selection)
{
    sdword i;
...
/*-----------------------------------------------------------------------------
    ...
    Return      :
----------------------------------------------------------------------------*/
bool_t processHoldingPattern(CommandToDo *command,SelectCommand *selection)
{
    sdword i,count;
                ...
                case COMMAND_NULL:
                    ...
                    else
                    {
                        if (command->ordertype.attributes & COMMAND_MASK_PASSIVE_ATTACKING)
                        {
                            ...
                            ship = command->selection->ShipPtr[0];
                            if ((bool_t)((ShipStaticInfo *)ship->staticinfo)->rotateToRetaliate)
                            {
                                if(ship->tacticstype != Evasive)
                                    aitrackSteadyShipDriftOnly(ship);
                            }
...
/*-----------------------------------------------------------------------------
    ...
    Return      : Returns allocated command containing the ship
----------------------------------------------------------------------------*/
SelectCommand *getShipAndItsCommandSelection(CommandLayer *comlayer,ShipPtr ship,bool_t *parade)
{
...
----------------------------------------------------------------------------*/
bool_t CommandInCommandLayer(CommandLayer *comlayer,CommandToDo *searchfor)
{
    Node *curnode = comlayer->todolist.head;
    CommandToDo *command;
...
void clChecksum(void)
{
    ...
    while (curnode != NULL)
    {
        ...
        if (cmd.attributes & COMMAND_MASK_FORMATION)
        {
            ...
            fwrite(&command->formation,sizeof(TypeOfFormation)+sizeof(bool_t),1,netlogfile);
