...
bool_t cCreateChannel(wchar_t *name, wchar_t *desc)
{
...
bool_t cJoinChannelRequest(wchar_t *name, wchar_t *desc)
{
    SetChannel(name,desc);
...
bool_t cCreateGame(wchar_t *name)
{

    return(FALSE);
}

bool_t cStartTitan(void)
{
    if (cChannelFSMState != CS_Idle)
    {
        return(TRUE);
...
sdword FindADefaultRoom(void)
{
    ...
    bool_t languagecriteria[FIND_DEFAULT_ROOM_TRIES] = { TRUE, TRUE,  FALSE };
    bool_t someonecriteria[FIND_DEFAULT_ROOM_TRIES] =  { TRUE, FALSE, FALSE };
