...
bool_t cmReallyPausedAll = FALSE;

bool_t cmActive=FALSE;
...
void cmAddFactory(struct Ship *ship,bool_t canBuildBigShips)
{
    shipsinprogress *sinprogress;
...
static bool_t resourcesHaveBeenAboveBefore = TRUE;
...
----------------------------------------------------------------------------*/
/*
bool_t cmShipDisabled(ShipType type)
{
    sdword index;
...
{
    sdword index;
    bool_t builtSomething = FALSE;
    bool_t capShipBuilt = FALSE;
...
void cmPauseJobs(char *string, featom *atom)
{
    sdword index;
    shipinprogress *progress;
    bool_t allpaused = TRUE, changed=FALSE;
...
----------------------------------------------------------------------------*/
bool_t cmClassCapCanCreateShip(ShipType shipType, shipsinprogress *factory, shipavailable *cmShipsAvail)
{
    ShipStaticInfo *shipStaticInfo;
    ShipClass shipClass;
    ShipRace shipRace;
    sdword index, nInThisClass;
    bool_t shipsQueuedUp[TOTAL_NUM_SHIPS];
    ...
    memset(shipsQueuedUp, 0, TOTAL_NUM_SHIPS * sizeof(bool_t));
...
bool_t cmIncrement(shipsinprogress *factory, uword index)
{
    shipinprogress *progress;
    bool_t flag = FALSE;
/*
...
bool_t cmDecrement(shipsinprogress *factory, uword index)
{
    shipinprogress *progress;
    bool_t flag = TRUE, skip = FALSE;
...
/*-----------------------------------------------------------------------------
    ...
    Return      :
----------------------------------------------------------------------------*/
void cmDrawArrow(regionhandle region, bool_t leftArrow)
{
...
bool_t cmCanBuildShipType(Ship *factoryship,ShipType shiptype,bool_t checkResearch)
{
...
bool_t cmBuildHotKey(keyindex key, bool_t shift)
{
    shipinprogress *progress;
    uword index;
    uword skey;
    bool_t buildhotkey = FALSE;
    ShipType ship;
...
bool_t cmDeterministicSanityCheck(cmDetermProgress_t* dprog)
{
    bool_t rval = TRUE;
...
void cmDeterministicBuildPause(
    bool_t pause, ShipType shipType, ShipRace shipRace, sdword playerIndex, ShipPtr creator)
{
    Node* node;
    cmDetermProgress_t* dprog;
