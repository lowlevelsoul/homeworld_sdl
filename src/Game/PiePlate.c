...
bool_t pieVisualizeExtents = FALSE;
#endif

vector pieScreen0, pieScreen1;
bool_t pieOnScreen = FALSE;
...
/*-----------------------------------------------------------------------------
    ...
    Return      : void
----------------------------------------------------------------------------*/
bool_t pieCancelPointSpecMode(sdword userID, ubyte *userData, sdword length)
{
...
----------------------------------------------------------------------------*/
bool_t pieSameOnScreenPoint(hmatrix *modelView, hmatrix *projection, vector *world0, vector *world1)
{
    sdword x0, x1, y0, y1;
...
void pieDistanceReadoutDraw(vector *movepoint, vector *origin, color c)
{
    ...
    glPushMatrix();
    glLoadIdentity();
    depthOn = (bool_t)glIsEnabled(GL_DEPTH_TEST);
...
void pieAllShipsToPiePlateDraw(real32 distance)
{
    ...
    bool_t bHeightPointDrawn   = FALSE,
...
/*-----------------------------------------------------------------------------
    ...
    Return      :
----------------------------------------------------------------------------*/
bool_t pieNeedSpecialAttackAndMoveColor()
{
    sdword i,j;
    CommandToDo *command;
...
/*-----------------------------------------------------------------------------
    ...
    Return      :
----------------------------------------------------------------------------*/
bool_t pieDestinationInGunRangeOfTargets()
{
    sdword i,j;
    vector distvec;
    real32 distSqr;
...
bool_t pieMovePointClipToLimits(real32 sizeX, real32 sizeY, real32 sizeZ, vector *pointA, vector *pointB)
{
    real32 discriminant;
