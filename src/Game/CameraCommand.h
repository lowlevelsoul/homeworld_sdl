...
typedef struct
{
    ...
    bool_t ViewEntireMissphere;
    Camera actualcamera;
    TransitionType transition;
    ActionType action;
    real32 transTimeStamp;
    sdword UserControlled;
    bool_t zoomInCloseAsPossible;
    real32 lockCameraTimeStamp;
    bool_t dontUseVelocityPredInChase;
    udword ccMode;
...
TI_API bool_t ccFocusOnFleet(CameraCommand *cameracommand);
TI_API bool_t ccFocusExact(CameraCommand *cameracommand,CameraCommand *focustocopy);
TI_API void ccFocusOnFleetSmooth(CameraCommand *cameracommand);
TI_API void ccFocusGeneral(CameraCommand *cameracommand,FocusCommand *focuscom, bool_t bCloseUp);
