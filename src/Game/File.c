...
/*-----------------------------------------------------------------------------
    ...
    Return      :
----------------------------------------------------------------------------*/
bool_t fileExistsInBigFile(char *fileName)
{
    udword fileIndex = 0;
    bigFileConfiguration *whereFound = NULL;
...
----------------------------------------------------------------------------*/
bool_t fileExists(char *_fileName, udword flags)
{
    char *fileName;
...
filehandle fileOpen(char *_fileName, udword flags)
{
    ...
    filehandle fh;
    bool_t usingBigfile    = FALSE;
    bool_t firstBufUse     = FALSE;
    bool_t localFileExists = FALSE;
...
bool_t fileCDROMPathSet(char *path)
{
#ifdef _WIN32
    char message[80];
...
bool_t fileOverrideBigPathSet(char *path)
{
    filePathMaxBufferSet(fileOverrideBigPath, path);
    return TRUE;
}

bool_t fileUserSettingsPathSet(char *path)
{
