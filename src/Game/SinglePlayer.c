...
bool_t hyperspaceAtEndOfSinglePlayerNIS = FALSE;
bool_t hyperspaceOverride               = FALSE;
bool_t singlePlayerGameLoadNewLevelFlag = FALSE;
bool_t singlePlayerHyperspacingInto     = FALSE;
bool_t spHoldHyperspaceWindow           = FALSE;
bool_t spBinkPlay                       = FALSE;
bool_t triggerNIS                       = FALSE;
bool_t spCollectResourcesAtEndOfMission = FALSE;
...
bool_t HYPERSPACE_MOTHERSHIP_ALWAYS_ARRIVES_FIRST = TRUE;
...
static void singlePlayerNISNamesGet(char **nisname, char **scriptname, bool_t *centreMothership, sdword nisNumber);
static void singlePlayerStartNis(char *nis, char *script, bool_t centre, hvector *positionAndAngle);
...
bool_t GetPointOfName(hvector *point,char *name)
{
...
bool_t GetStartPointPlayer(hvector *startpoint)
{
...
bool_t AreNotInParadeConstraint(Ship *ship)
{
    if (ship->specialFlags & SPECIAL_SinglePlayerInParade)
    {
        return FALSE;
...
bool_t AllDoingExceptMe(udword state, Ship* me)
{
    Node* objnode;
    Ship* ship;
...
bool_t NoneDoingHS(udword state)
{
    InsideShip* insideship;
    Node* objnode;
    Ship* ship;
...
bool_t UpdateHyperspacingShip(Ship *ship,bool_t midLevel)
{
    bool_t hyperspacedout = FALSE;
    ShipSinglePlayerGameInfo *shipSinglePlayerGameInfo = ship->shipSinglePlayerGameInfo;
...
bool_t UpdateHyperspacingShips(void)
{
    Node *objnode;
    Ship *ship;
    bool_t shipsAllHyperspacedAway = TRUE;
    bool_t result;
...
bool_t OrientHyperspacingShips(void)
{
    Node *objnode;
    Ship *ship;
    vector mothershipheading;
    bool_t shipsoriented = TRUE;
    Ship *mothership = universe.curPlayerPtr->PlayerMothership;
...
bool_t UpdateArrivingShip(Ship *ship,hvector *topoint,bool_t midLevel)
{
    ShipSinglePlayerGameInfo *shipSinglePlayerGameInfo = ship->shipSinglePlayerGameInfo;
    bool_t thisShipArrived = FALSE;
...
bool_t UpdateArrivingShips()
{
    InsideShip *insideShip;
    Ship *ship;
    Node *node = singlePlayerGameInfo.ShipsInHyperspace.head;
    bool_t allShipsArrived = TRUE;
    hvector startpoint;
...
bool_t ShipAlreadyHyperspaceOut(Ship *ship)
{
...
void singlePlayerPostInit(bool_t loadingSaveGame)
{
    if (!loadingSaveGame)
    {
...
void CleanupHyperspacingShips(bool_t FreeShips,bool_t cleanupAllPlayersShips)
{
    InsideShip *insideShip;
    Ship *ship;
    Node *node = singlePlayerGameInfo.ShipsInHyperspace.head;
...
{
    bool_t addedShipToHyperspace = FALSE;
    sdword i;
...
//                soundEventStopTrack(SongNumber, SPEECH_HYPERSPACE_DELAY + 1.5f);
        ...
        case HYPERSPACE_SHIPSLEAVING:
            ...
            {
                bool_t shipsoriented = OrientHyperspacingShips();
...
/*-----------------------------------------------------------------------------
    ...
    Return      : void
----------------------------------------------------------------------------*/
void singlePlayerNISNamesGet(char **nisname, char **scriptname, bool_t *centreMothership, sdword nisNumber)
{
    ...
    *centreMothership = (bool_t)nisPlayRelativeToMothership[nisNumber];
}
...
----------------------------------------------------------------------------*/
bool_t singlePlayerNISletNamesGet(char **nisname, char **scriptname, sdword nisletNumber)
{
...
/*-----------------------------------------------------------------------------
    ...
    Return      : void
----------------------------------------------------------------------------*/
void singlePlayerStartNis(char *nisname, char *scriptname, bool_t centreMothership, hvector *positionAndAngle)
{
...
----------------------------------------------------------------------------*/
bool_t spFindCameraAttitude(vector *position)
{
