...
/*-----------------------------------------------------------------------------
    ...
    Description : Convert a string to a bool_t.  Looks for nonzero numbers, "TRUE" or "YES"
    Inputs      : boolString - string to convert
    Outputs     :
    Return      : TRUE or FALSE
----------------------------------------------------------------------------*/
bool_t scriptStringToBool(char *boolString)
{
...
{
    *((bool_t *)dataToFillIn) = scriptStringToBool(field);
}
...
----------------------------------------------------------------------------*/
bool_t parseLine(char *line,char **returnName,char **returnValue)
{
    char *name;
    char *value;
    sdword length;
    ...
    for (;;)
    {
        ...
        if (parseLine(line,&name,&value))
        {
            ...
            if (foundentry != NULL)
            {
                ...
                foundentry->setVarCB(directory,value,structureToFillIn + ((uintptr_t)foundentry->offset1 - (uintptr_t)foundentry->offset2) );
            }
        }
    }
            ...
            case GT_OPENGAME:
                ...
                else
                {
                    ...
                    if (foundentry != NULL)
                    {
                        ...
                        foundentry->setVarCB("",value,structureToFillIn + ((uintptr_t)foundentry->offset1 - (uintptr_t)foundentry->offset2) );
                    }
                }
                break;
            default:
