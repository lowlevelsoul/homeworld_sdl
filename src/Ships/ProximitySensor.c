...
bool_t proxShouldDrawOverlay(Ship *ship);
...
bool_t proximityCanPlayerSeeShip(Player *player,Ship *shipInQuestion)
{
    sdword mask = 0x01;
    if(player == NULL)
        return FALSE;
...
----------------------------------------------------------------------------*/
bool_t ProximitySensorPingTimeout(struct ping *hellaPing, SpaceObj *user, char *userData, bool_t bRemoveReferences)
{
